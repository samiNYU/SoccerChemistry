...
val prob_goal_df = slicerGoal.transform(spark.read.parquet(
  "hdfs:///user/sa5064/Project/Data/FinalData/Predictions/GBT/Label_Goal_GBTPredictions.snappy.parquet")).select(
...
val prob_concede_df = slicerConcede.transform(spark.read.parquet(
  "hdfs:///user/sa5064/Project/Data/FinalData/Predictions/GBT/Label_Concede_GBTPredictions.snappy.parquet")).select(
  $"index",$"match_id",
  regexp_replace(
...
val prob_possession_df = slicerPossession.transform(spark.read.parquet(
  "hdfs:///user/sa5064/Project/Data/FinalData/Predictions/GBT/Label_Possession_GBTPredictions.snappy.parquet")).select(
  $"index",$"match_id",
  regexp_replace(
...
val prob_lose_possession_df = slicerLosePossession.transform(spark.read.parquet(
  "hdfs:///user/sa5064/Project/Data/FinalData/Predictions/GBT/Label_LosePossession_GBTPredictions.snappy.parquet")).select(
  $"index",$"match_id",
  regexp_replace(
...
val newFilePath = "hdfs:///user/sa5064/Project/Results/GBT/"
...
fs.rename(new Path(tempFilePath+tempFileName),new Path(newFilePath+"resultsGBT.snappy.parquet"))
...
prob_goal_df.unpersist()
prob_concede_df.unpersist()
prob_possession_df.unpersist()
prob_lose_possession_df.unpersist()
combined.unpersist()
prevs.unpersist()
diffs.unpersist()
VAEP.unpersist()
